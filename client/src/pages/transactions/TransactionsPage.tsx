import { Button, Dialog, DialogActions, DialogBody, DialogContent, DialogSurface, DialogTrigger, Drawer, Input, Menu, MenuItem, MenuList, MenuPopover, MenuTrigger } from '@fluentui/react-components';
import { ArrowImportRegular, ChevronDownRegular, DeleteRegular, DismissCircleRegular, SearchRegular } from "@fluentui/react-icons";
  ...
  return (
    <>
        ...
        <div className='m-2 flex flex-row gap-2 flex-wrap'>
          ...
          <div className="flex flex-row gap-2">
            {selectedTransactionIds.length > 0 &&
              <Dialog>
                <DialogTrigger>
                  <CustomButton icon={<DeleteRegular />} variant="danger">{t('app.delete')}</CustomButton>
                </DialogTrigger>
                <DialogSurface>
                  <DialogBody>
                    <DialogContent>
                    ...
                    <DialogActions>
                      <Button appearance="subtle" onClick={(e) => {
                        ...
                        if (dialog) {
                          const overlay = dialog.parentElement?.querySelector('[data-overlay]');
            ...
            {account && <CustomButton icon={<ArrowImportRegular />} onClick={() => setImportDrawerOpen(true)}>{t('app.import')}</CustomButton>}
      ...
      <Drawer 
        ...
        onOpenChange={(e, data) => setImportDrawerOpen(data.open)}
        position="end"
        size="large"
      >
        <div className="p-4">
          <h2 className="text-xl font-semibold mb-4">{t('app.import')}</h2>
